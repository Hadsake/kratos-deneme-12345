...
exports.run = (client, message, args) => {
  ...
  let terfiler = guild.channels.find('name', 'kayıtlar');
  if (!terfiler) return message.reply('`kayıtlar` kanalını bulamıyorum.');
  ...
  const embed = new Discord.RichEmbed()
    ...
    return guild.channels.get(terfiler.id).send(embed);
};
